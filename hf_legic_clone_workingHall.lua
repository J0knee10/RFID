...
local function xorme(hex, xor, index)
    if ( index >= 23 ) then
        return ('%02x'):format(bxor( tonumber(tostring(hex), 16) , tonumber(tostring(xor), 16) ))
    else
        return hex
    end
end
...
local function writeOutputBytes(bytes, outfile)
    ...
    for i = 1, #bytes do
        fho:write(string.char(tonumber(tostring(bytes[i]), 16)))
    end
    fho:close()
...
-- safer getSegmentData: normalize byte types up-front
local function getSegmentData(bytes, start, index)
    local segment = {}

    -- ensure bytes are strings where needed and numbers for bit ops
    local b0s = tostring(bytes[start] or "00")
    local b1s = tostring(bytes[start + 1] or "00")
    local b2s = tostring(bytes[start + 2] or "00")
    local b3s = tostring(bytes[start + 3] or "00")
    -- numeric values for bit32.extract
    local b0 = tonumber(b0s, 16) or 0
    local b1 = tonumber(b1s, 16) or 0
    local b2 = tonumber(b2s, 16) or 0
    local b3 = tonumber(b3s, 16) or 0

    -- raw header as hex strings (keeps compatibility with print code)
    segment[0] = string.format("%02x %02x %02x %02x", b0, b1, b2, b3)

    -- flag = high nibble of byte1 (as hex char of high nibble)
    segment[1] = string.format("%x", bit32.extract(b1, 4, 4))

    -- valid = bit 6 of byte1
    segment[2] = bit32.extract(b1, 6, 1)

    -- last = bit 7 of byte1
    segment[3] = bit32.extract(b1, 7, 1)

    -- len = (byte0) + (bit0-3 of byte1 << 8)
    local high_nibble = bit32.extract(b1, 0, 4)  -- low 4 bits of byte1
    segment[4] = tonumber(string.format("%01x%02x", high_nibble, b0), 16)

    -- wrp (write protected) = byte2 as number
    segment[5] = b2

    -- wrc (write control) = bits 4-6 of byte3 (3 bits)
    segment[6] = bit32.extract(b3, 4, 3)

    -- rd (read disabled) - bit7 of byte3
    segment[7] = bit32.extract(b3, 7, 1)

    -- crc byte 4: return as hex string for compatibility with other code
    local crc_byte = bytes[start + 4] or "00"
    if type(crc_byte) == "number" then
        segment[8] = string.format("%02x", crc_byte)
    else
        segment[8] = string.format("%02x", tonumber(tostring(crc_byte), 16) or 0)
    end

    -- segment index
    segment[9] = index

    -- address (index) of the segment CRC byte in the original bytes array
    segment[10] = start + 4
...
local function CheckKgh(bytes, segStart, segEnd)
    if (bytes[8] == '9f' and bytes[9] == 'ff' and bytes[13] == '11') then
        local i
        local data = {}
        segStart = tonumber(tostring(segStart), 10)
        segEnd = tonumber(tostring(segEnd), 10)
        local dataLen = segEnd - segStart - 5
        --- gather creadentials for verify
       local WRP = tostring(bytes[(segStart + 2)] or "00")   -- keep as hex-string
        -- convert the header byte to a number for bit ops
        local hdr3_num = tonumber(tostring(bytes[segStart + 3] or "00"), 16) or 0
        local WRC = string.format("%02x", bit32.extract(hdr3_num, 4, 3))
        local RD  = string.format("%02x", bit32.extract(hdr3_num, 7, 1))
        local XX  = "00"
        cmd = tostring(bytes[1] or "00")..tostring(bytes[2] or "00")..tostring(bytes[3] or "00")..tostring(bytes[4] or "00")..WRP..WRC..RD..XX
...
local function displaySegments(bytes)
    ...
    repeat
       ...
       KGH = CheckKgh(bytes, start, (start + Seg[4]))
        -- or simply: KGH = CheckKgh(bytes, start, (start + Seg[4]))
...
local function writeToTag(plainBytes)
    ...
    for i = 0, (#SegCrcs - 1) do
        ...
        -- normalize header bytes to numbers first
        local header_low = tonumber(tostring(plainBytes[SegCrcs[i] - 4] or "00"), 16) or 0
        local header_hi_byte = tonumber(tostring(plainBytes[SegCrcs[i] - 3] or "00"), 16) or 0
        -- extract the high nibble / bits used for length (same logic as getSegmentData)
        local high_nibble = bit32.extract(header_hi_byte, 0, 4)  -- low 4 bits
        -- build segment length as (high_nibble << 8) + low byte
        local segLen = (high_nibble * 0x100) + header_low
